...
## M16D — miner web distribution (2026-09-11)

Distribution and onboarding only. **No inference, no settlement, no beta-v2
activation, no reward change, no migration, no schema change.** The ledger,
settlements, M14C and M16B economic units and beta-v2 scheduling are untouched,
and the pending calibration cleanup and beta cutover are still pending.

**The problem.** USAGE Miner was not a part of the product a normal person
could obtain. 0.4.5 existed only as a GitHub Actions artifact, and the website
advertised 0.3.0 — because the release manifest was a constant generated by the
miner's packaging script back when the miner lived in this repository. After
the repository split, that pipeline was severed and nothing noticed: the
constant simply kept describing whichever build the platform was last cut
alongside.

**The fix, in one sentence:** the site reads what a published release actually
contains. `src/lib/miner/distribution.ts` selects the newest non-draft release
of `Deniscoke/USAGE-Miner` and takes version, size, checksum, publication date
and signing state from that release's own `release.json` and `SHA256SUMS.txt`.
A checksum therefore always describes the bytes behind the link printed beside
it. Drafts are excluded because a draft is a 404 for everyone but the owner.
Failure — no release, rate limit, unreachable, unparsable — resolves to the
last build known to be downloadable, marked `source: "pinned"`, never to an
error page and never to an invented version. `signed` is true only when the
release's own manifest says so, and that manifest is written by the workflow
step that has just verified Authenticode against the expected publisher.

**Surfaces.** `/download` is public and canonical (`/miners/install` redirects
to it), linked from the homepage as a secondary CTA behind sign-up, from the
dashboard, and from the providers page — where a connected provider with no
online miner now gets an explicit next step, because connecting a provider is
not mining and a page that implies otherwise is making a promise the account
cannot keep. The dashboard shows not installed / offline / online, and stops
advertising a download to somebody already mining. `update_available` and
`update_required` come from the published version and
`MINIMUM_MINER_VERSION`; a build NEWER than anything published says nothing at
all, which is the owner's own machine today. What an old build is actually
allowed to do is still decided server-side per request — the page's wording has
no authority over it.

**Release pipeline (miner repository).** A tag push published nothing:
`inputs.publish != false` was meant to read as "unless told otherwise", but
`inputs` is null on a tag push and GitHub casts null and false alike, so it was
false for every tag. Signing was skipped by the same expression. Absence now
reads as "the default applies". Published assets lost their version numbers
(`USAGE-Miner-Windows-x64-Setup.exe`) so a link survives a release;
`scripts/stage-release.mjs` renames, recomputes `SHA256SUMS.txt` and
`release.json` over the staged copies, publishes nothing that is a build
byproduct, and refuses to stage a release with no installer. Releases are still
created as DRAFTS — publishing binaries stays a click a person makes.

**Not built, deliberately.** No `usage://` protocol handler: a registered
scheme is callable by any page the user visits, so a link that launches a local
AI tool is a larger security surface than the convenience justifies (the design
...
a release publishes a manifest that says so.
